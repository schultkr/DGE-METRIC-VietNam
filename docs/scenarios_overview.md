...
| Scenario | Short name | What it models | Key shock type |
|---|---|---|---|
| EE under PDP8 | `EE_PDP8` | Industry and services EE improvements consistent with PDP8 efficiency targets | Energy productivity shock (`exo_AI`) |
| EE under EU Directive 10 | `EE_Directive10` | Stronger EE ambition aligned with EU Directive 10 equivalent for industry and services | Higher `exo_AI` + additional investment cost |
| EE_PDP8 without BESS | `EE_PDP8_PV_BESS_NoBESS` | EE_PDP8 PV+BESS variant with BESS-specific channels reset to baseline | BESS channels zeroed out |
| Directive 10 without BESS | `EE_Directive10_NoBESS` | EE_Directive10 with BESS-specific channels reset | BESS channels zeroed out |
...
**Model representation:** Finance instruments are translated into reduced-form model shocks — primarily cost-of-capital (`exo_r_G`, `exo_r_FDI`) and investment-price channels (`exo_P_K`). The model does not represent individual bond instruments or guarantee balance sheets, but captures their aggregate effect on investment and consumption.
...
| `ExcelFiles/PDP8/Vietnam_EnergyExpert_ScenarioInputs*.xlsx` | Expert EE inputs (source for CreateEEScenariosFromExpertInputs.m) |
...
- [Scenario design (technical)](scenario.md)
- [EE scenario design (implementation detail)](ee_scenario_design.md)
- [Finance instruments feasibility](finance_instruments_comments_feasibility.md)
