...
- **Non-technical overview:** [What is DGE-METRIC?](../policy/overview.md)
...
> **Landmine — energy-efficiency variable shape.** Two shapes of EE variable
> exist in this codebase's history: a legacy single regional `EE_1`/`exo_EE_1`
> and a sector-specific refactor `EE_sec_reg`/`exo_EE_sec_reg` (sec = 1..4).
> As of 2026-07-14, **only the legacy regional shape (`EE_1`/`exo_EE_1`) is
> declared in `ModFiles/`** (confirmed by grep — no `EE_sec_reg` variables
> exist anywhere in `ModFiles/`). Do not write new scenario or analysis code
> assuming the sector-specific refactor is active; check
> `git grep -n "EE_.*_1"` in `ModFiles/` before relying on either shape.
...
The model is calibrated to a Vietnam 2026 baseline using:
- GSO input-output table 2019 (sectoral shares)
- EVN annual reports (energy sector)
- IEA WEO 2025 (CAPEX, investment paths)
- EDGAR 2024 (baseline emissions)

See [Calibration](calibration.md) and [Data sources](data_sources.md) for details.

---

## Core structure (technical)
...
The diagram below shows the four transmission channels through which energy policy shocks — the ETS cap, energy efficiency targets, the renewable investment path, and green finance — propagate through the model to GDP, investment, consumption, and emissions.
...
In contrast to models with endogenous fiscal feedback rules, tax rates on labor and capital are assumed to be constant over time. As a result, government financing does not respond endogenously to climate damages, economic fluctuations, or changes in emissions for those tax bases. Any variation in government revenues due to climate policy arises exclusively through changes in tax bases and ETS revenues.

The consumption tax $\tau^C_t$ is the exception, via an endogenous instrument
`tauCEndo_reg`. It mirrors the energy-efficiency variable `EE_reg`: a compile-time
switch (`BaselineScenario` in `ModFiles/Equations/government.mod`, toggled by
`Functions/Miscellaneous/ModelSetup/change_mod_file.m`, not a per-period exogenous
shock) makes `tauCEndo_reg` — and hence $\tau^C_t$ — endogenous in the **Baseline**
scenario, solved so that $G_t/Y_t$ hits a target ratio (defaulting to the calibrated
Baseline ratio, adjustable via `exo_targetGY_reg`). Every other scenario instead holds
$\tau^C_t$ fixed at the exact path solved in Baseline (`exo_tauC_reg`, set
programmatically), with an additional `exo_tauCScen_reg` shock left free for
scenario-specific consumption-tax policy experiments layered on top of that path.
