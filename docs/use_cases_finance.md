...
As of June 2026, the green finance scenarios are designed and the WACF workbook (`Vietnam_Green_Finance_Scenarios_April2026.xlsx`) is complete. The next implementation step is:

1. Translate WACF assumptions into shock paths in `ExcelFiles/ModelScenarios5Sectorsand1Regions.xlsx`
2. Reactivate `PDP8_GF_A`, `PDP8_GF_B`, `PDP8_GF_C` in `RunSimulations.m` (currently commented out)
3. Run and generate output CSVs, then update figures with `scripts/reporting/GenerateFinanceSimulationResultsFigures.m`

See [Finance instruments feasibility](finance_instruments_comments_feasibility.md) for the full comment-by-comment implementation assessment and [Finance instruments short note](finance_instruments_comments_feasibility_short.md) for the summary.
...
- [Finance instruments feasibility (full)](finance_instruments_comments_feasibility.md)
- [Finance instruments feasibility (short)](finance_instruments_comments_feasibility_short.md)
