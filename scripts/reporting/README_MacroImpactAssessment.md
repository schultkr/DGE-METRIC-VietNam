...
`Functions/Miscellaneous/Reporting/report_version_suffix.m`: `_replication_fix` by default (the
`sSensitivity` value `RunSimulations.m` writes with the `Model*_replication_fix.xlsx` workbooks, in
...
Names below are the scenario stems; each is read as `<stem>_replication_fix.csv` by default (see above).

- Figures 1–2: `Baseline`, `EE_Dir10_full`, `EE_Dir10_full_NoBESS`, `EE_RTS_prerev_95GW` (`DataVersion = "replication_fix"`, falls back to `_replication`, then plain).
