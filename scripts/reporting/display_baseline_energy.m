...
require_vars(ds, ["Year", "Y_1", "P_1", ...
    "I_H_2_1", "I_G_2_1", "P_INV_2_1", ...
    "I_H_3_1", "I_G_3_1", "P_INV_3_1", ...
    "K_2_1", "K_3_1", ...
    "Q_2_1", "Q_3_1", "EE_1"], 'baseline data');

plotMask = ds.Year >= plotStartYear & ds.Year <= plotEndYear;
if ~any(plotMask)
    error('display_baseline_energy:noPlotYears', ...
...
invRenNom = (ds.I_H_3_1 + ds.I_G_3_1 + ds.I_FDI_3_1) .* ds.P_INV_3_1;
invFosNom = (ds.I_H_2_1 + ds.I_G_2_1 + ds.I_FDI_2_1) .* ds.P_INV_2_1;
...
if nPeriods < 1
    error('display_baseline_energy:noPeriods', ...
...
function data = add_year_variable(data, startYear)
    ...
    if ~ismember('Time', data.Properties.VariableNames)
        error('display_baseline_energy:missingYear', ...
...
function require_vars(data, vars, dataName)
    ...
    if ~isempty(missing)
        error('display_baseline_energy:missingVariables', ...
