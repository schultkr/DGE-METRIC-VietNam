...
% scenario). DataVersion below picks which variant to prefer; the shared
% pipeline (generate_ee_simulation_results_figures.m) falls back to the
% other variant automatically if the preferred one isn't available for
% every required scenario (reported via fprintf), so this never has to be
% re-checked scenario by scenario.
...
%
% Used in: IWH_Report_Macro Impact Assessment.docx, Figure 3 (top panel:
...
figureScenarioConfig.DataVersion = "replication";
