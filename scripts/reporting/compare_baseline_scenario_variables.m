...
scenarioSpecs = table( ...
    ["EE_PDP8_ref"; "EE_Dir10_full"; 'EE_Dir10_full_NoBESS'], ...
    ["PDP8 reference EE"; "Directive 10 (full)"; 'Directive 10 without BESS'], ...
...
% Add/remove rows here to choose variables.
% `Variable` accepts either:
%   - a single variable name, e.g. "Y_1"
%   - a derived expression, e.g. "C_1+I_1", "C_1-I_1", "E_1/Y_1"
% Derived expressions must use exactly one operator type per row.
plotSpecs = table( ...
    ["Y_1";   "C_1";          "I_1";       "E_1";       ...
     "EE_1";  "Q_3_1";        "K_3_1";     "PE_1"; "Q_I_5_1_2"], ...
    ["GDP";   "Consumption";  "Investment"; "Emissions"; ...
     "Energy efficiency"; "Renewable production"; ...
     "Renewable capital"; "Emission price"; 'Energy services'], ...
    ["index"; "index";        "index";     "index";     ...
     "index"; "index";        "index";     "level"; 'index'], ...
    ["pct_deviation"; "pct_deviation"; "pct_deviation"; "pct_deviation"; ...
     "pct_deviation"; "pct_deviation"; "pct_deviation"; "difference"; 'pct_deviation'], ...
    'VariableNames', {'Variable', 'Label', 'Transform', 'Deviation'});

outDir = fullfile(repoRoot, 'Figures', 'ScenarioComparisons');
if ~exist(outDir, 'dir')
    mkdir(outDir);
end
...
requiredVars = collect_required_vars(plotSpecs);
...
for iScen = 1:height(scenarioSpecs)
    ...
    sCsv = fullfile(repoRoot, 'ExcelFiles', 'Output', sName + sversion + ".csv");
    if ~isfile(sCsv)
    ...
    require_vars(sData, ["Year"; requiredVars], char(sName) + " data");
...
if isempty(loadedScenarios)
    error('compare_baseline_scenario_variables:noScenarioData', ...
...
if isempty(commonYears)
    error('compare_baseline_scenario_variables:noCommonYears', ...
...
if exportOptions.SinglePlotsForSlides
    for iPlot = 1:height(plotSpecs)
        ...
        fig = make_figure(sprintf('%s: Baseline and scenarios', spec.Label), ...
            [100 100 860 560]);
        ax = axes('Parent', fig);
        baselineSeries = evaluate_plot_series(baseline, string(spec.Variable));
        plot(ax, years, compute_transform_series(baselineSeries, ...
            baselineSeries, string(spec.Transform), plotOptions), ...
        ...
        for iScen = 1:numel(loadedScenarios)
            ...
            scenarioSeries = evaluate_plot_series(sData, string(spec.Variable));
            yVals = compute_transform_series(scenarioSeries, ...
                baselineSeries, string(spec.Transform), plotOptions);
        ...
        for iScen = 1:numel(loadedScenarios)
            sData = loadedScenarios(iScen).Data;
            scenarioSeries = evaluate_plot_series(sData, string(spec.Variable));
            [dVals, ~] = compute_deviation(scenarioSeries, ...
                baselineSeries, string(spec.Deviation), ...
...
function specs = normalize_plot_specs(specs)
    ...
    if ~isempty(missing)
        error('compare_baseline_scenario_variables:badPlotSpecs', ...
    ...
    if any(bad)
        error('compare_baseline_scenario_variables:badTransform', ...
    ...
    if any(badDeviation)
        error('compare_baseline_scenario_variables:badDeviation', ...
    ...
    if numel(unique(specs.Variable)) ~= numel(specs.Variable)
        error('compare_baseline_scenario_variables:duplicateVariables', ...
...
function require_vars(data, vars, dataName)
    ...
    if ~isempty(missing)
        error('compare_baseline_scenario_variables:missingVariables', ...
            'Missing required variable(s) in %s: %s', ...
            dataName, strjoin(missing, ', '));
    end
end

function requiredVars = collect_required_vars(specs)
    allVars = strings(0, 1);
    for i = 1:height(specs)
        allVars = [allVars; extract_expression_variables(specs.Variable(i))]; %#ok<AGROW>
    end
    requiredVars = unique(allVars, 'stable');
end

function values = evaluate_plot_series(data, expr)
    expr = strtrim(string(expr));
    vars = extract_expression_variables(expr);
    kind = detect_expression_kind(expr);

    switch kind
        case "single"
            values = data.(char(vars(1)));

        case "sum"
            values = zeros(height(data), 1);
            for i = 1:numel(vars)
                values = values + data.(char(vars(i)));
            end
        ...
        case "ratio"
            if numel(vars) ~= 2
                error('compare_baseline_scenario_variables:badExpression', ...
                    'Ratio expressions must have exactly two terms: %s', expr);
            end
            num = data.(char(vars(1)));
            den = data.(char(vars(2)));
            values = safe_divide(num, den);

        otherwise
            error('compare_baseline_scenario_variables:badExpression', ...
                'Unsupported expression: %s', expr);
    end
end

function vars = extract_expression_variables(expr)
    expr = strtrim(string(expr));
    vars = split(expr, ["+", "-", "/"]);
    vars = strtrim(vars);
    ...
    if isempty(vars)
        error('compare_baseline_scenario_variables:badExpression', ...
            'Expression is empty: %s', expr);
    end
end

function kind = detect_expression_kind(expr)
    expr = char(strtrim(string(expr)));
    hasPlus = contains(expr, '+');
    hasMinus = contains(expr, '-');
    hasDiv = contains(expr, '/');
    opCount = double(hasPlus) + double(hasMinus) + double(hasDiv);

    if opCount == 0
        kind = "single";
    elseif opCount > 1
        error('compare_baseline_scenario_variables:badExpression', ...
            'Mixing operators is not supported in one expression: %s', expr);
    elseif hasPlus
        kind = "sum";
    elseif hasMinus
        kind = "difference";
...
function values = compute_transform_series(seriesValues, baselineValues, transformMode, plotOptions)
...
%               plotOptions.PctDeviationVisibilityThreshold);
        ...
        otherwise
            error('compare_baseline_scenario_variables:unsupportedTransformMode', ...
