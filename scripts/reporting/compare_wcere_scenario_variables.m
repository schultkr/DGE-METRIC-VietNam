...
plotSubfolder = 'ScenarioComparisons/WCEREPDP8';
...
plotSpecs = table( ...
    ...
    ["GDP";   "Consumption";  "Investment"; "Emissions"; ...
     ...
     "Renewable capital"; "Emission price"; "WACC renewables"], ...
...
baseline.WACC_3_1 = baseline.r_F_3_1 .* baseline.P_K_3_1./ baseline.P_INV_3_1([1 1:end-1]);
baseline.GDP_growth = baseline.Y(1:end) ./ baseline.Y([1 1:end-1]);
...
for iScen = 1:height(scenarioSpecs)
    ...
    sData.WACC_3_1 = sData.r_F_3_1 .* sData.P_K_3_1./ sData.P_INV_3_1([1 1:end-1]);
...
function csvPath = resolve_output_csv(outputDir, stem, mustExist)
    ...
    if ~outputDirExists
        error('compare_baseline_scenario_variables:missingOutputDir', ...
    ...
    if isempty(availableCsv)
        error('compare_baseline_scenario_variables:missingBaselineCsv', ...
