% create_baseline_from_path_definition_lite  Build runnable Baseline sheet from path definitions only.
%
% Run from the repository root:
%   run('scripts/maintenance/create_baseline_from_path_definition_lite.m')
%
...
% create_baseline_from_user_input_file in dedicated-path mode, but it disables
...
% Reset every column a later step treats as additive ("read existing, add an
% increment, write back") so this script is idempotent regardless of rerun
% history -- see the matching comment in create_baseline_from_user_input_file.m.
...
% existing, add an increment") ONLY if they already exist -- see the matching
% comment in create_baseline_from_user_input_file.m.
...
if any(missingIdx)
    ...
    warning('create_baseline_from_path_definition_lite:IndexedTrajectoryCarryForward', ...
