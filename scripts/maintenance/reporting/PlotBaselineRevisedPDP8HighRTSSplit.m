% PlotBaselineRevisedPDP8HighRTSSplit
...
%   run('scripts/reporting/PlotBaselineRevisedPDP8HighRTSSplit.m')
...
if ~isfile(workbookPath)
    error('PlotBaselineRevisedPDP8HighRTSSplit:MissingWorkbook', ...
        'Workbook not found:\n  %s', workbookPath);
end
...
if isempty(hdrRow)
    error('PlotBaselineRevisedPDP8HighRTSSplit:MissingHeaderRow', ...
...
if isempty(iYear) || isempty(iCapHigh) || isempty(iCap) || isempty(iCF)
    error('PlotBaselineRevisedPDP8HighRTSSplit:MissingColumns', ...
...
if any(isnan(capHigh))
    ...
    if numel(iKnown) < 2
        warning('PlotBaselineRevisedPDP8HighRTSSplit:InsufficientHighAnchors', ...
