% create_baseline_from_user_input_file  Build runnable Baseline sheet from user input.
%
% Run from the repository root:
%   run('scripts/maintenance/create_baseline_from_user_input_file.m')
%
% Two input modes are supported:
%   1) Dedicated path workbook (default): ExcelFiles/ScenarioPathDefinition.xlsx
%      Reads sheet "Input Scenario" when present, otherwise "Baseline", and
%      rebuilds the runnable "Baseline" sheet.
%   2) Legacy combined workbook: ModelSimulationandCalibration...xlsx
%      Rebuilds the runnable "Baseline" sheet from the legacy source workbook.
...
%                    target capital stock per year, summed directly from
%                    Investment.csv), so the investment ratio is a direct
%                    reflection of capital-stock development.
% Override with env var DGE_TARGET_IY_METHOD.
TargetInvestmentMethod = string(targetConfig.defaultMethod);
envTargetInvestmentMethod = strtrim(getenv('DGE_TARGET_IY_METHOD'));
if ~isempty(envTargetInvestmentMethod)
    TargetInvestmentMethod = string(envTargetInvestmentMethod);
end
...
EnableIndustrialPVtoEECoupling = true;
...
EnableGovernmentRTSSectorPVShocks = true;
...
% If available, use parameters directly from calibration workbook.
UseCalibrationParametersIfAvailable = true;
...
% Optional: set no-ETS emissions path from PDP8 non-power baseline emissions.
EnableNonPowerNOETSBaselinePath = true;
...
legacySourceWorkbook = targetConfig.calibrationWorkbook;
...
if UseCalibrationParametersIfAvailable
    [deltaPVCalib, foundDeltaPV] = read_calibration_parameter_value(legacySourceWorkbook, 'deltaPV_p');
    [phiKPV0Calib, foundphiKPV0] = read_calibration_parameter_value(legacySourceWorkbook, 'phiKPV0_p');
    [deltaMaintFossilCalib, foundDeltaMaintFossil] = read_calibration_parameter_value(legacySourceWorkbook, 'delta_2_1_p');
    [deltaMaintRenewableCalib, foundDeltaMaintRenewable] = read_calibration_parameter_value(legacySourceWorkbook, 'delta_3_1_p');
    if foundDeltaPV
...
switch lower(char(Mode))
    case 'dedicated_path'
        ...
        dedicatedInputs.deltaMaintFossil = DeltaMaintFossil;
        dedicatedInputs.deltaMaintRenewable = DeltaMaintRenewable;
        dedicatedInputs.targetInvestmentMethod = char(TargetInvestmentMethod);
        dedicatedInputs.indexProxyBaseCapitalShareFossil = ...
            targetConfig.indexProxyBaseCapitalShareFossil;
        dedicatedInputs.indexProxyBaseCapitalShareRenewable = ...
        ...
        write_nonpower_noets_to_path_workbook(dedicatedInputs);
        ...
        report_terminal_va_share_consistency(dedicatedInputs, yearsWritten, repoRoot);
        remove_new_sheets(targetWorkbook, existingSheetNames, {'Baseline'});

        fprintf('\nCreateBaselineFromUserInputFile complete.\n');
        fprintf('  Mode: dedicated_path\n');
    ...
    case 'legacy_combined'
        ...
        summary = update_baseline_excel( ...
            'SourceWorkbook', legacySourceWorkbook, ...
            'TargetWorkbook', targetWorkbook, ...
            'HardcodedSheet', 'Baseline', ...
            'RefreshInputSheetsFromSource', true, ...
            'BootstrapFromLegacy', true, ...
            'ForceBootstrap', false, ...
            'Visible', false);
    ...
    otherwise
        error('create_baseline_from_user_input_file:InvalidMode', ...
            'Unknown Mode "%s". Use "dedicated_path" or "legacy_combined".', Mode);
end

function dedicatedInputs = import_dedicated_path_inputs(sourceWorkbook)
if ~isfile(sourceWorkbook)
    error('create_baseline_from_user_input_file:SourceNotFound', ...
        'Dedicated path workbook not found:\n  %s', sourceWorkbook);
end

% ScenarioPathDefinition layout:
% Required input rows are located by their import keys in columns A:D.
% Year columns use D:AD (configured below), i.e. base year 2025 plus the
% simulated horizon -- compute_sector_growth_factors and the PDP8
% target-investment path both need the 2025 base year present in
% dedicatedInputs.years to compute t-1 growth/GDP anchoring.
...
srcSheet = resolve_path_source_sheet(sourceWorkbook, srcSheet, fallbackSourceSheets);
...
if any(isnan(years))
    ...
    warning('create_baseline_from_user_input_file:YearHeaderInferred', ...
        ['Could not parse all year headers in %s row 9 (E:AD). ' ...
         'Using inferred sequence %d:%d.'], srcSheet, years(1), years(end));
end

gvaGrowth = read_required_labeled_paths(sourceWorkbook, srcSheet, srcStartCol, srcEndCol, ...
    {'gva_growth_total'; 'Total value-added growth factor'});
vaShares = read_required_labeled_paths(sourceWorkbook, srcSheet, srcStartCol, srcEndCol, ...
    {'va_share_1', 'va_share_2', 'va_share_3', 'va_share_4', 'va_share_5'; ...
     'VA share - Primary', 'VA share - Fossil', 'VA share - Renewables', ...
     'VA share - Secondary', 'VA share - Tertiary'});
empGrowth = read_required_labeled_paths(sourceWorkbook, srcSheet, srcStartCol, srcEndCol, ...
    {'emp_growth_total'; 'Total employment growth factor'});
empShares = read_required_labeled_paths(sourceWorkbook, srcSheet, srcStartCol, srcEndCol, ...
    {'emp_share_1', 'emp_share_2', 'emp_share_3', 'emp_share_4', 'emp_share_5'; ...
     'Employment share - Primary', 'Employment share - Fossil', ...
     'Employment share - Renewables', 'Employment share - Secondary', ...
     'Employment share - Tertiary'});

if numel(gvaGrowth) ~= numel(years) || numel(empGrowth) ~= numel(years) || ...
    size(vaShares, 2) ~= numel(years) || size(empShares, 2) ~= numel(years)
    error('create_baseline_from_user_input_file:UnexpectedSourceLayout', ...
...
if ~isempty(availableSheets)
    ...
    error('create_baseline_from_user_input_file:SourceSheetNotFound', ...
...
if ~isequal(years, yearsEmp)
    error('create_baseline_from_user_input_file:YearAlignmentMismatch', ...
        'GVA and employment growth year ranges are not aligned.');
end

headers = readcell(targetWorkbook, 'Sheet', 'Baseline', 'Range', '1:1');
if isempty(headers)
    error('create_baseline_from_user_input_file:MissingBaselineHeaders', ...
...
% Reset every column that a later step treats as additive ("read existing,
% add an increment, write back") so this script is idempotent regardless of
% rerun history. Without this, apply_government_rts_sector_pv_shocks reuses
% whatever exo_GA_4_1/5_1 already has as its new base level (only resetting
% to phiKPV0 when the existing value is empty/<=0), and apply_vneep3_ee_targets
% unconditionally adds its increment to whatever's already in exo_AI_4_1_2/5_1_2
% and exo_GA_4_1/5_1 -- running this script twice in a row against the same
% target file compounds those columns upward each time.
headers = reset_additive_baseline_columns(targetWorkbook, 'Baseline', headers, nYears, ...
    {'exo_GA_3_1', 'exo_GA_4_1', 'exo_GA_5_1', 'exo_AI_4_1_2', 'exo_AI_5_1_2'});
...
for iSub = 1:size(gY, 1)
    ...
    if isempty(iColGY) || isempty(iColGN)
        error('create_baseline_from_user_input_file:MissingGrowthColumn', ...
...
function headersOut = reset_additive_baseline_columns(targetWorkbook, targetSheet, headersIn, nYears, varNames)
% Zero out columns that downstream steps treat as additive/reusable ("read
% existing, add an increment") ONLY if they already exist -- if a column
% doesn't exist yet, the function that eventually creates it (via
% ensure_baseline_column_exists) already initializes it to zero, so there is
% nothing to reset. This makes the additive chain (RTS/PV coupling, VNEEP3
% targets) produce the same result regardless of whether this script has
% already been run against this target file before.
headersOut = headersIn;
zeros1 = zeros(nYears, 1);
for i = 1:numel(varNames)
    varName = varNames{i};
    iCol = find_header_index(headersOut, varName);
    if isempty(iCol)
        continue
    end
    write_series_to_baseline_column(targetWorkbook, headersOut, varName, zeros1, 1:nYears, nYears);
end
end
...
if isfield(dedicatedInputs, 'sourceWorkbook') && isfield(dedicatedInputs, 'sourceSheet')
    rowNum = find_path_row_by_import_key(dedicatedInputs.sourceWorkbook, dedicatedInputs.sourceSheet, 'exo_E_NOETS_1_1');
    if ~isempty(rowNum)
        fprintf(['  Non-power baseline NOETS path is sourced from ScenarioPathDefinition ' ...
            '(row %d); skipping direct Baseline overwrite.\n'], rowNum);
        return
    end
end
...
if ~isfield(dedicatedInputs, 'nonPowerNOETSWorkbook') || isempty(dedicatedInputs.nonPowerNOETSWorkbook)
    warning('create_baseline_from_user_input_file:NonPowerNOETSWorkbookMissing', ...
...
if ~isfile(wbPath)
    warning('create_baseline_from_user_input_file:NonPowerNOETSWorkbookNotFound', ...
...
try
    ...
    warning('create_baseline_from_user_input_file:NonPowerNOETSReadFailed', ...
...
if isempty(yearCol)
    warning('create_baseline_from_user_input_file:NonPowerNOETSYearMissing', ...
...
if isempty(seriesCol)
    warning('create_baseline_from_user_input_file:NonPowerNOETSSeriesMissing', ...
...
if numel(ySrc) < 2
    warning('create_baseline_from_user_input_file:NonPowerNOETSInsufficientData', ...
...
if any(~isfinite(vAligned)) || any(vAligned <= 0)
    warning('create_baseline_from_user_input_file:NonPowerNOETSInvalidAlignedSeries', ...
...
if numel(trimmedConverted) ~= nYears
    warning('create_baseline_from_user_input_file:NonPowerNOETSLengthMismatch', ...
...
if rtsOk && isfield(plan, 'genIndustrialGWh') && isfield(plan, 'genCommercialGWh')
    ...
    warning('create_baseline_from_user_input_file:RTSGenDataMissing', ...
    ...
    if ~isempty(gaSec) && ~isempty(gaTer) && all(isfinite(gaSec)) && all(isfinite(gaTer)) && gaSec(1) > 0 && gaTer(1) > 0
        ...
        if isempty(pvSeries)
            warning('create_baseline_from_user_input_file:IndustrialPVPathMissing', ...
        ...
        if ~(isfinite(pvMin) && isfinite(pvMax)) || pvMax <= pvMin + 1e-12
            warning('create_baseline_from_user_input_file:IndustrialPVNoVariation', ...
...
if isempty(etaSec) || isempty(etaTer)
    warning('create_baseline_from_user_input_file:MissingEEColumns', ...
...
if ~ok
    warning('create_baseline_from_user_input_file:RTSSectorPVPlanMissing', '%s', msg);
    return
end
...
if ~(deltaPV > 0 && deltaPV <= 1)
    error('create_baseline_from_user_input_file:InvalidDeltaPVForRTS', ...
        'deltaPV for RTS investment-index conversion must be in (0,1]. Got %.6f', deltaPV);
end
if ~(phiKPV0 > 0)
    error('create_baseline_from_user_input_file:InvalidPhiKPV0ForRTS', ...
...
if ~(isfinite(subsecInd) && subsecInd == floor(subsecInd) && subsecInd >= 1)
    error('create_baseline_from_user_input_file:InvalidIndustrialPVGASubsector', ...
...
if ~(isfinite(subsecCom) && subsecCom == floor(subsecCom) && subsecCom >= 1)
    error('create_baseline_from_user_input_file:InvalidCommercialPVGASubsector', ...
...
if isempty(yCsv)
    msg = 'RTS split CSV has no usable Year data.';
    return
end

[isIn, idxPos] = ismember(yearsRef(:), yCsv(:));
if ~all(isIn)
    % Years beyond the RTS split CSV's own coverage: assume stable RTS --
    % no further rooftop-solar capacity/generation growth once the expert
    % plan data runs out -- rather than failing PV->EE coupling entirely.
    % Hold the nearest available year's data flat (clamps to the CSV's
    % first/last year for years outside its range).
    missingYears = yearsRef(~isIn);
    fprintf(['  RTS split CSV does not cover %d requested year(s) (%s); ' ...
        'holding nearest available year flat (stable-RTS assumption).\n'], ...
        sum(~isIn), mat2str(missingYears(:)'));
    for k = find(~isIn(:))'
        [~, nearestIdx] = min(abs(yCsv(:) - yearsRef(k)));
        idxPos(k) = nearestIdx;
    end
end
...
if any(~isfinite(idxStock)) || any(idxStock <= 0)
    error('create_baseline_from_user_input_file:InvalidPVStockIndex', ...
...
if any(idxInvest < 0)
    warning('create_baseline_from_user_input_file:NegativePVInvestmentIndex', ...
...
iCol = find_header_index(headers, varName);
if isempty(iCol)
    warning('create_baseline_from_user_input_file:MissingOptionalBaselineColumn', ...
        'Baseline column "%s" not found; skipping write for this path.', varName);
    return
end

if numel(seriesRow) ~= nYearsExpected
    error('create_baseline_from_user_input_file:OptionalPathLengthMismatch', ...
...
if any(~isfinite(seriesRow))
    error('create_baseline_from_user_input_file:InvalidOptionalPathValues', ...
...
if needsAnchor
    iAnchor = find(sourceYears == anchorYear, 1, 'first');
    if isempty(iAnchor)
        error('create_baseline_from_user_input_file:MissingAnchorYear', ...
...
if contains(ruleNorm, 'binary') || contains(ruleNorm, 'value 0')
    if any(seriesRow < 0)
        error('create_baseline_from_user_input_file:InvalidBinaryInput', ...
...
if contains(ruleNorm, 'additive') || (contains(ruleNorm, 'index') && contains(ruleNorm, 'index 1') && contains(ruleNorm, 'minus'))
    if any(seriesRow <= 0)
        error('create_baseline_from_user_input_file:NonPositiveIndex', ...
...
error('create_baseline_from_user_input_file:UnknownConversionRule', ...
...
% Compute PDP8 target-I/Y paths and write them to the ScenarioPathDefinition source sheet.
...
[ok, fossilSeries, renewSeries, msg] = compute_pdp8_target_investment_series( ...
    ...
    dedicatedInputs.deltaMaintFossil, dedicatedInputs.deltaMaintRenewable, ...
    dedicatedInputs.targetInvestmentMethod, ...
    dedicatedInputs.indexProxyBaseCapitalShareFossil, ...
    dedicatedInputs.indexProxyBaseCapitalShareRenewable);
if ~ok
    warning('create_baseline_from_user_input_file:PDP8TargetIYWriteSkipped', '%s', msg);
    return
end
fprintf('  PDP8 target-I/Y method: %s\n', dedicatedInputs.targetInvestmentMethod);
...
for i = 1:numel(targetRows)
    ...
    if isempty(rowNum)
        warning('create_baseline_from_user_input_file:MissingTargetIYRow', ...
...
fprintf('  Wrote PDP8 target-I/Y shares to ScenarioPathDefinition sheet "%s".\n', ...
    dedicatedInputs.sourceSheet);
end

function write_nonpower_noets_to_path_workbook(dedicatedInputs)
% Compute non-power NOETS index path and write it to ScenarioPathDefinition.

if ~isfield(dedicatedInputs, 'enableNonPowerNOETSBaselinePath') || ~dedicatedInputs.enableNonPowerNOETSBaselinePath
    return
end

[ok, idxSeries, anchorYear, msg] = compute_nonpower_noets_index_series(dedicatedInputs);
if ~ok
    warning('create_baseline_from_user_input_file:NonPowerNOETSWriteSkipped', '%s', msg);
    return
end

key = 'exo_E_NOETS_1_1';
rowNum = ensure_path_row_by_import_key(dedicatedInputs.sourceWorkbook, dedicatedInputs.sourceSheet, key);

writecell({key}, dedicatedInputs.sourceWorkbook, 'Sheet', dedicatedInputs.sourceSheet, ...
    'Range', sprintf('B%d', rowNum));
writecell({'log(index/index(1))'}, dedicatedInputs.sourceWorkbook, 'Sheet', dedicatedInputs.sourceSheet, ...
    'Range', sprintf('C%d', rowNum));

rowRange = sprintf('%s%d:%s%d', dedicatedInputs.sourceStartCol, rowNum, dedicatedInputs.sourceEndCol, rowNum);
writematrix(reshape(idxSeries, 1, []), dedicatedInputs.sourceWorkbook, ...
    'Sheet', dedicatedInputs.sourceSheet, 'Range', rowRange);

fprintf(['  Wrote non-power baseline NOETS path to ScenarioPathDefinition sheet "%s" ' ...
    '(row %d, anchor year %d).\n'], dedicatedInputs.sourceSheet, rowNum, anchorYear);
end
...
function rowNum = ensure_path_row_by_import_key(sourceWorkbook, srcSheet, importKey)
rowNum = find_path_row_by_import_key(sourceWorkbook, srcSheet, importKey);
if ~isempty(rowNum)
    return
end
...
for r = 10:size(meta, 1)
    rowVals = meta(r, :);
    if all(cellfun(@local_is_empty_cell, rowVals))
        rowNum = r;
        break
    end
end
...
if isempty(rowNum)
    error('create_baseline_from_user_input_file:NoAvailablePathRow', ...
        'No available row found in A1:D300 of %s/%s for import key %s.', ...
        sourceWorkbook, srcSheet, importKey);
end
end

function tf = local_is_empty_cell(v)
if isempty(v)
    tf = true;
elseif ismissing(v)
    tf = true;
...
function [ok, idxSeries, anchorYear, msg] = compute_nonpower_noets_index_series(dedicatedInputs)
ok = false;
msg = '';
idxSeries = [];
anchorYear = 2025;
...
if ~isfield(dedicatedInputs, 'nonPowerNOETSWorkbook') || isempty(dedicatedInputs.nonPowerNOETSWorkbook)
    msg = 'No non-power NOETS workbook configured; skipping ScenarioPathDefinition write for exo_E_NOETS_1_1.';
    return
end

wbPath = dedicatedInputs.nonPowerNOETSWorkbook;
if ~isfile(wbPath)
    msg = sprintf('Non-power NOETS workbook not found: %s. Skipping ScenarioPathDefinition write for exo_E_NOETS_1_1.', wbPath);
    return
end
...
try
    ...
    msg = sprintf('Failed to read %s: %s. Skipping ScenarioPathDefinition write for exo_E_NOETS_1_1.', wbPath, ME.message);
    return
end
...
if isempty(yearCol)
    msg = sprintf('Workbook %s has no Year column. Skipping ScenarioPathDefinition write for exo_E_NOETS_1_1.', wbPath);
    return
end
...
if isempty(seriesCol)
    msg = sprintf(['Could not find non-power baseline series column (expected %s) in %s. ' ...
        'Skipping ScenarioPathDefinition write for exo_E_NOETS_1_1.'], seriesName, wbPath);
    return
end
...
if numel(ySrc) < 2
    msg = sprintf('Non-power baseline series in %s has insufficient valid points. Skipping ScenarioPathDefinition write for exo_E_NOETS_1_1.', wbPath);
    return
end
...
vAligned = interp1(yUniq, vUniq, sourceYears, 'linear', 'extrap');
if any(~isfinite(vAligned)) || any(vAligned <= 0)
    msg = 'Aligned non-power baseline series is invalid after interpolation. Skipping ScenarioPathDefinition write for exo_E_NOETS_1_1.';
    return
end
...
if useCapitalStock
    ...
    if any(missingIdx)
        ...
        warning('create_baseline_from_user_input_file:IndexedTrajectoryCarryForward', ...
...
if numel(factors) ~= numel(years)
    error('create_baseline_from_user_input_file:GDPGrowthLengthMismatch', ...
        'GVA growth path length (%d) must match year path length (%d).', numel(factors), numel(years));
end

idxBase = find(years == gdpBaseYear, 1, 'first');
if isempty(idxBase)
    error('create_baseline_from_user_input_file:GDPBaseYearMissing', ...
...
function series = read_required_labeled_paths(sourceWorkbook, srcSheet, srcStartCol, srcEndCol, labelCandidates)
% Read required path rows by exact import key or display label.
% Each column of labelCandidates identifies one required output row.
labelBlock = readcell(sourceWorkbook, 'Sheet', srcSheet, 'Range', 'A1:D300');
seriesRows = cell(size(labelCandidates, 2), 1);

for iPath = 1:size(labelCandidates, 2)
    candidates = labelCandidates(:, iPath);
    candidates = candidates(~cellfun(@isempty, candidates));
    candidateNorms = string(cellfun(@normalize_label_text, candidates, ...
        'UniformOutput', false));
    rowMatches = false(size(labelBlock, 1), 1);

    for r = 1:size(labelBlock, 1)
        for c = 1:size(labelBlock, 2)
            value = labelBlock{r, c};
            if (ischar(value) || isstring(value)) && ...
                    any(normalize_label_text(value) == candidateNorms)
                rowMatches(r) = true;
                break
            end
        end
    end
    matchingRows = find(rowMatches);

    if isempty(matchingRows)
        error('create_baseline_from_user_input_file:MissingRequiredInputKey', ...
            'Required input "%s" was not found in columns A:D of sheet "%s".', ...
            strjoin(string(candidates), '" or "'), srcSheet);
    elseif numel(matchingRows) > 1
        error('create_baseline_from_user_input_file:DuplicateRequiredInputKey', ...
            'Required input "%s" occurs on multiple rows (%s) in sheet "%s".', ...
            strjoin(string(candidates), '" or "'), ...
            strjoin(string(matchingRows), ', '), srcSheet);
    end

    rowRange = sprintf('%s%d:%s%d', ...
        srcStartCol, matchingRows, srcEndCol, matchingRows);
    seriesRows{iPath} = reshape(readmatrix(sourceWorkbook, 'Sheet', srcSheet, ...
        'Range', rowRange), 1, []);
    if iPath > 1 && numel(seriesRows{iPath}) ~= numel(seriesRows{1})
        error('create_baseline_from_user_input_file:RequiredInputWidthMismatch', ...
            'Required input "%s" has a different path width in sheet "%s".', ...
            candidates{1}, srcSheet);
    end
end
series = vertcat(seriesRows{:});
end
...
if emitWarnings && ~isempty(labelCandidates)
    warning('create_baseline_from_user_input_file:MissingOptionalInputLabel', ...
...
if isempty(yearsWritten)
    warning('create_baseline_from_user_input_file:NoYearsWritten', ...
...
if isempty(idxTerminal)
    warning('create_baseline_from_user_input_file:TerminalYearNotInInput', ...
...
if ~isfile(outputCsv)
    warning('create_baseline_from_user_input_file:MissingBaselineOutput', ...
...
if ~ismember('Year', sim.Properties.VariableNames)
    warning('create_baseline_from_user_input_file:MissingYearColumn', ...
...
if isempty(row)
    row = height(sim);
    warning('create_baseline_from_user_input_file:TerminalYearMissingInOutput', ...
...
if any(isnan(simNominalVA)) || sum(simNominalVA) <= 0
    warning('create_baseline_from_user_input_file:MissingVAColumnsInOutput', ...
...
function [growth, yearsOut] = compute_sector_growth_factors(totalGrowth, shares, years, targetStartYear)
    if size(shares, 2) ~= numel(totalGrowth) || numel(years) ~= numel(totalGrowth)
        error('create_baseline_from_user_input_file:GrowthShareLengthMismatch', ...
    ...
    if any(totalGrowthFactors <= 0)
        error('create_baseline_from_user_input_file:NonPositiveGrowth', ...
            ['Growth input implies nonpositive factors. Provide either factors (>0), ' ...
             'decimal rates (e.g., 0.02), or percent rates (e.g., 2 for 2%%).']);
    end

    if any(shares(:) <= 0)
        error('create_baseline_from_user_input_file:NonPositiveShare', ...
    ...
    if any(abs(shareSums - 1) > 1e-4)
        error('create_baseline_from_user_input_file:InvalidShares', ...
            'Sector shares must sum to 1 in each year.');
    end

    idxStart = find(years >= targetStartYear, 1, 'first');
    if isempty(idxStart)
        error('create_baseline_from_user_input_file:NoYearsAfterStart', ...
            'No source years >= %d were found in Baseline.', targetStartYear);
    end
    if idxStart == 1
        error('create_baseline_from_user_input_file:MissingPreStartYear', ...
...
if iCol < 1
    error('create_baseline_from_user_input_file:InvalidColumnIndex', ...
...
try
    ...
    warning('create_baseline_from_user_input_file:HeaderFormatCopyFailed', ...
...
if isempty(baselineTable) || width(baselineTable) < 2
    error('create_baseline_from_user_input_file:BaselineNotBuilt', ...
...
if ~any(strcmpi(varNames, 'Time'))
    error('create_baseline_from_user_input_file:BaselineMissingTime', ...
