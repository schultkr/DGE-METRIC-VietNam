...
@# for reg in 1:Regions
    @# for sec in 1:Sectors
        @# for subsec in Subsecstart[sec]:Subsecend[sec]
            ...
            I_P_@{subsec}_@{reg} ${I^P_{s,r}}$ (long_name = 'non-productive private investment sector @{subsec} region @{reg}')   
            K_P_@{subsec}_@{reg} ${I^P_{s,r}}$ (long_name = 'non-productive private capital stock sector @{subsec} region @{reg}')   
            ...
            A_INV_@{subsec}_@{reg} ${\xi^{I}_{s,r}}$ (long_name = 'marginal efficiency of investment shock (MEI)')
    ...
    I_G_@{reg} ${I^G_{r}}$ (long_name = 'public net invesmtent')
...
@# for sec in 1:Sectors
    ...
    @# for subsec in Subsecstart[sec]:Subsecend[sec]
        ...
        @# for reg in 1:Regions
            ...
            exo_I_P_@{subsec}_@{reg} ${\eta^{I^P,s,r}}$ (long_name = 'exogenous private investment shock sector @{subsec} region @{reg}')
            ...
            exo_lKRGTarget_@{subsec}_@{reg} ${\eta^{l^{KRG},s,r}}$ (long_name = 'switch: 0=exo r_G, 1=target total K via r_G (fossil/RE only)')
            exo_KRGTarget_@{subsec}_@{reg} ${\eta^{KRG,s,r}}$ (long_name = 'K target log deviation from K0 when using r_G targeting')
            ...
            exo_sFDIShare_@{subsec}_@{reg} ${\eta^{s^{FDI},s,r}}$ (long_name = 'K_FDI/K target share in [0,1] — used when exo_lFDIShare=1')
            ...
            exo_A_INV_@{subsec}_@{reg} ${\eta^{\xi^{I},s,r}}$ (long_name = 'MEI shock path from baseline (set programmatically in scenarios)')
...
@# for sec in 1:Sectors
    ...
    @# for subsec in Subsecstart[sec]:Subsecend[sec]
        ...
        @# for reg in 1:Regions
            ...
            phiKP0_@{subsec}_@{reg}_p ${\phi^{K^P,0}_{s,r}}$ (long_name = 'base-period private investment share of output')
            delta_@{subsec}_@{reg}_p ${\delta^{K}_{s,r}}$ (long_name = 'depreciation rate for capital')
            s_G_@{subsec}_@{reg}_p ${s^{G}_{s,r}}$ (long_name = 'initial savings rate for public sector invesmtent')
            ...
            targetIY0_@{subsec}_@{reg}_p ${\overline{IY}_{s,r}}$ (long_name = 'SS investment-to-GDP ratio (calibrated at steady state)')
        @# endfor
    @# endfor
@# endfor
