function change_mod_file(sScenario,sSubsecstart,sSubsecend,sRegions,...
    sSimulation, sExoNX, sCapandTrade, sClimRegional, sClimNational, sTargetBase)
    % function change_mod_file(sScenario,sSubsecstart,sSubsecend,sRegions) to change the macroprocessor
    ...
    iposline = find(cellfun(@(x) contains(x,"@# define NEndogenous"), temp));
    if length(iposline) == 1 && iposline > 0 
        if contains(sScenario, 'Baseline')
            temp{iposline} = '@# define NEndogenous = 0';
        else
            temp{iposline} = '@# define NEndogenous = 1';
        end
        [fid] = fopen('DGE_Model.mod', 'w');
        fwrite(fid, strjoin(temp, '\n'));
        fclose(fid);    
    ...
    iposline = find(cellfun(@(x) contains(x,"@# define BaselineScenario"), temp));
    if length(iposline) == 1 && iposline > 0
        if contains(sScenario, 'Baseline')
            temp{iposline} = '@# define BaselineScenario = 1';
        else
            temp{iposline} = '@# define BaselineScenario = 0';
        end
        [fid] = fopen('DGE_Model.mod', 'w');
        fwrite(fid, strjoin(temp, '\n'));
        fclose(fid);
    else
        error('No unique match for @# define BaselineScenario')
    end
