...
% find positions of intermediate input shocks
...
% find positions of technology shocks
temp = arrayfun(@(y) arrayfun(@(x) ['P_K_' num2str(y) '_' num2str(x)], 1:inbregions_p, 'UniformOutput', false), 1:imaxsec_p, 'UniformOutput', false);
casPKvars = [temp{:}];
[lSelectPKVars, iposPKVars] = ismember(casPKvars, cellstr(M_.endo_names));
...
% find positions of tax subsidies to capital
temp = arrayfun(@(y) arrayfun(@(x) ['exo_tauKF_' num2str(y) '_' num2str(x)], 1:inbregions_p, 'UniformOutput', false), 1:imaxsec_p, 'UniformOutput', false);
castauKFShocks = [temp{:}];
[lSelecttauKFShocks, ipostauKFShocks] = ismember(castauKFShocks,cellstr(M_.exo_names));

% find positions of tax subsidies to capital
temp = arrayfun(@(y) arrayfun(@(x) ['exo_phiK_' num2str(y) '_' num2str(x)], 1:inbregions_p, 'UniformOutput', false), 1:imaxsec_p, 'UniformOutput', false);
castauphiKShocks = [temp{:}];
[lSelectphiKShocks, iposphiKShocks] = ismember(castauphiKShocks,cellstr(M_.exo_names));

% find positions of interest rate shocks
temp = arrayfun(@(y) arrayfun(@(x) ['exo_P_K_' num2str(y) '_' num2str(x)], 1:inbregions_p, 'UniformOutput', false), 1:imaxsec_p, 'UniformOutput', false);
casPKShocks = [temp{:}];
[lSelectPKShocks, iposPKShocks] = ismember(casPKShocks,cellstr(M_.exo_names));

% find positions of public capital stock shocks
temp = arrayfun(@(y) arrayfun(@(x) ['exo_K_G_' num2str(y) '_' num2str(x)], 1:inbregions_p, 'UniformOutput', false), 1:imaxsec_p, 'UniformOutput', false);
casKGShocks = [temp{:}];
[lSelectKGShocks, iposKGShocks] = ismember(casKGShocks,cellstr(M_.exo_names));
casIGShocks = casKGShocks;
lSelectIGShocks = lSelectKGShocks;
iposIGShocks = iposKGShocks;

% find positions of interest rate shocks
temp = arrayfun(@(y) arrayfun(@(x) ['exo_PKexo_' num2str(y) '_' num2str(x)], 1:inbregions_p, 'UniformOutput', false), 1:imaxsec_p, 'UniformOutput', false);
casrShocks = [temp{:}];
[lSelectrexoShocks, iposrexoShocks] = ismember(casrShocks,cellstr(M_.exo_names));

% find positions of sectoral labour
temp = arrayfun(@(y) arrayfun(@(x) ['r_' num2str(y) '_' num2str(x)], 1:inbregions_p, 'UniformOutput', false), 1:imaxsec_p, 'UniformOutput', false);
casrVars = [temp{:}];
[lSelectrVars, iposrVars] = ismember(casrVars, cellstr(M_.endo_names));
