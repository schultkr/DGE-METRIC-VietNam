% create_raw_excel_input_file is a Matlab script to create the
% create_raw_excel_input_file is a Matlab script to create the
...
%% Define sectors
% casSectors = {'Producing Industries'; 'Energy'; 'Services'...
%               }';
casSectors = {'Non-Energy'; 'Energy'...
              }';
inbsectors_p = length(casSectors);
%% Define subsectors
% casSubSectors = {'Producing Industries'; 'Fossil'; 'Renewables'; 'Services'...
%               };
casSubSectors = {'Non-Energy'; 'Energy'...
              };
inbsubsectors_p = length(casSubSectors);
%% Define regions
casRegions = {'VNM'; 'CHN'; 'IND'};%; 'EU'};
%casRegions = {'EU'; 'Case Study'};
...
%% Create the workbook
sWorkBookName = ['ModelSimulationandCalibration' num2str(inbsubsectors_p) 'Sectorsand' num2str(inbregions_p) 'Regions' sversion '.xlsx'];


addpath(genpath(fullfile(sPathWD, 'Functions')))
sThisFolder = fileparts(mfilename('fullpath'));
run(fullfile(sThisFolder, 'define_sheets_input_file.m'));
...
for icosheet = 1:size(strSheet,2)
    ...
    if isstruct(strSheet(icosheet).Categories)
        ...
        for icosubsheet = 1:size(strsubsheets,2)
            ...
            dat_range = [get_excel_column(icostartcol) '1:' get_excel_column(icostartcol+inbcol-1) num2str(inbrow)]; % Example range
        ...
        for icocol = 1:inbcol
            if icocol == ivaluecol
                dat_range = [get_excel_column(icocol) '1:' get_excel_column(icocol) '1']; % Example range
                                dat_range = [get_excel_column(icocol) '1:' get_excel_column(icocol) '1']; % Example range
                rngObj = exlSheet1.Range(dat_range);
                rngObj.Value = strSheet(icosheet).Categories(1, icocol);                      
                dat_range = [get_excel_column(icocol) '2:' get_excel_column(icocol) num2str(inbrow)]; % Example range
                                dat_range = [get_excel_column(icocol) '2:' get_excel_column(icocol) num2str(inbrow)]; % Example range
                rngObj = exlSheet1.Range(dat_range);
        ...
        for icorow = 1:inbrow
            dat_range = ['A' num2str(icorow) ':' get_excel_column(inbcol) num2str(icorow)]; % Example range
                        dat_range = ['A' num2str(icorow) ':' get_excel_column(inbcol) num2str(icorow)]; % Example range
            rngObj = exlSheet1.Range(dat_range);
