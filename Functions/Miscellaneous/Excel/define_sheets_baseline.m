...
nExoPathCols = numel(casSectorEmissions) + numel(casFossilProd) + numel(casFossilExp) + ...
    ...
    numel(casIGShareFlag) + numel(casIGShare) + numel(casFDIShareFlag) + numel(casFDIShare);
...
exoPathBlock = [ ...
    ...
    ones(nRows, numel(casPublicRates)), ...
...
casData = arrayfun(@(x) num2str(x), [(2:(nRows + 1))' yearVals baseBlock growthBlock indexBlock exoPathBlock targetBlock], 'UniformOutput', false);
