...
casCategoriesHeader = [{'Time'}, {'Year'}, {'exo_LF_1'}, {'exo_NLF_1'}, casGrowthRatesY, casGrowthRatesN, ...
    ...
    casPublicCapital, casTargetInv, casTargetIY, casLTargetIY, casTargetGY, casLAddEE, casInvestmentPrices, casPublicRates, casEmissionPrice, ...
...
if ~exist('endYear', 'var'), endYear = 2051; end
if ~exist('defaultYear', 'var'), defaultYear = 2050; end
firstYear = baseYear + 1;
nRows = endYear - baseYear;
assert(defaultYear >= firstYear && defaultYear <= endYear, ...
    'defaultYear must lie between %d and %d.', firstYear, endYear);
...
nExoPathCols = numel(casSectorEmissions) + numel(casFossilProd) + numel(casFossilExp) + ...
    numel(casPublicCapital) + numel(casTargetInv) + numel(casTargetIY) + numel(casLTargetIY) + numel(casTargetGY) + numel(casLAddEE) + ...
...
exoPathBlock = [ ...
    ...
    zeros(nRows, numel(casIGShareFlag)), ...
    zeros(nRows, numel(casIGShare)), ...
    zeros(nRows, numel(casFDIShareFlag)), ...
    zeros(nRows, numel(casFDIShare))...
    ];

% Years after the default year inherit its default values. This extends the
% workbook through 2051 while keeping 2050 as the terminal-data default.
defaultRow = defaultYear - baseYear;
if defaultRow < nRows
    baseBlock(defaultRow + 1:end, :) = repmat(baseBlock(defaultRow, :), nRows - defaultRow, 1);
    growthBlock(defaultRow + 1:end, :) = repmat(growthBlock(defaultRow, :), nRows - defaultRow, 1);
    indexBlock(defaultRow + 1:end, :) = repmat(indexBlock(defaultRow, :), nRows - defaultRow, 1);
    exoPathBlock(defaultRow + 1:end, :) = repmat(exoPathBlock(defaultRow, :), nRows - defaultRow, 1);
end

yearVals = (firstYear:endYear)';
