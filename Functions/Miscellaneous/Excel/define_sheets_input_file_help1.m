function casCategories = define_sheets_input_file_help1(casParams, inbregions_p, inbsubsectors_p, inbsectors_p)
    % function casCategories = define_sheets_input_file_help1(casParams, inbregions_p, inbsubsectors_p, inbsectors_p)
    ...
    for icoparam = 1:length(casParams)  
        ...
        lsectoralorigin = casParams{7, icoparam};
        lregionalorigin = casParams{8, icoparam};
        if lregional && lsubsectoral && ~lsectoral && lsectoralorigin && ~lregionalorigin
            carowtemp = {['Parameter values for ' sDes ], '', ''};
            for icosubsec = 1:inbsubsectors_p
                for icoreg = 1:inbregions_p
                    for icosec = 1:inbsectors_p
                        carowtemp = [carowtemp;...
                                     {[sParam '_' num2str(icosubsec) '_' num2str(icoreg)  '_' num2str(icosec) '_p'], sValue, [sDes ' in sector ' num2str(icosubsec) ' and region ' num2str(icoreg) ' from sector ' num2str(icosec)]};...
                                     ];%#ok
                    end
                end
            end            
        elseif lregional && lsubsectoral && ~lsectoral && ~lsectoralorigin && lregionalorigin
            carowtemp = {['Parameter values for ' sDes ], '', ''};
            for icosubsec = 1:inbsubsectors_p
                for icoreg = 1:inbregions_p
                    for icoregm = 1:inbregions_p
                        carowtemp = [carowtemp;...
                                     {[sParam '_' num2str(icosubsec) '_' num2str(icoreg)  '_' num2str(icoregm) '_p'], sValue, [sDes ' in sector ' num2str(icosubsec) ' and region ' num2str(icoreg) ' from region ' num2str(icoregm)]};...
                                     ];%#ok
                    end
                end
            end            
