...
function [strys, strpar, strexo] = compute_exogenous_y_production(strys, strpar, strexo)
            ...
            for icoreg = 1:strpar.inbregions_p
                ...
                if icosubsec >0%~= strpar.iSubsecFossil_p
                    if strpar.lTargetY_p == 1
                        % Price-weighted VA target in the model numeraire:
                        % P0_s*Y0_s (calibrated FCgva). Real Y adjusts with P_s.
                        strys.(['Y_' ssubsec '_' sreg]) = strpar.(['P0_' ssubsec '_' sreg '_p']) .* strpar.(['Y0_' ssubsec '_' sreg '_p']) * exp(strexo.(['exo_' ssubsec '_' sreg])) / strys.(['P_' ssubsec '_' sreg]);
                    elseif strpar.lTargetY_p == 2
                        % Real Y target = Y0_s (calibrated real output, deflated by P0_s).
                        strys.(['Y_' ssubsec '_' sreg]) = strpar.(['Y0_' ssubsec '_' sreg '_p']) * exp(strexo.(['exo_' ssubsec '_' sreg]));
