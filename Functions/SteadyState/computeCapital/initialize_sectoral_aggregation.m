function [strys, strexo] = initialize_sectoral_aggregation(strys, strpar, strexo)
    ...
    for icoreg = 1:strpar.inbregions_p
        ...
        for icosec = 1:strpar.inbsectors_p
            ...
            strys.(['Q_A_F_' ssec '_' sreg]) = strpar.(['omegaQA_' ssec '_' sreg '_p']) *  strys.(['A_F_' ssec '_' sreg])^(strpar.etaQ_p-1) * ...
                ...
                strys.(['Q_U_' sreg]) - strys.(['Q_PV_' sreg]) * (icosec == strpar.iSecEnergy_p);
            ...
            strys.(['Q_A_' ssec '_' sreg]) = strys.(['Q_A_I_' ssec '_' sreg]) + ...
                ...
                (strpar.iSecHouse_p == icosec) * (strys.(['IH_' sreg]) * strys.(['PH_' sreg])+strys.(['I_PV_' sreg])) / strys.(['P_A_' ssec '_' sreg]);
            ...
            for icosubsec = strpar.(['substart_' ssec '_p']):strpar.(['subend_' ssec '_p'])
                ...
                strpar.(['rhoA_' ssec '_p']) = (strpar.(['etaQA_' ssec '_p']) - 1)/ strpar.(['etaQA_' ssec '_p']);
                strys.(['Q_D_' ssubsec '_' sreg]) = strpar.(['omegaQ_' ssubsec '_' sreg '_p']) * strys.(['A_D_' ssubsec '_' sreg])^(strpar.(['etaQA_' ssec '_p'])-1) *...
