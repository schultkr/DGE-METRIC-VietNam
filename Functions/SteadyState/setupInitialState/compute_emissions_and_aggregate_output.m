function [strys, strpar] = compute_emissions_and_aggregate_output(strys, strpar)
    %COMPUTE_EMISSIONS_AND_AGGREGATE_OUTPUT Computes regional and national emissions and aggregate outputs Q_A.
%
% Inputs:
...
%   - strys  : updated with Q_A and E values at sectoral, regional, and national level
    ...
    for icoreg = 1:strpar.inbregions_p
        ...
        for icosec = 1:strpar.inbsectors_p
            ...
            for icosubsec = strpar.(['substart_' ssec '_p']) : strpar.(['subend_' ssec '_p'])
                ...
                strys.(['E_NOETS_' sreg]) = strys.(['E_NOETS_' sreg]) + strys.(['E_NOETS_' ssubsec '_' sreg]);
        ...
        strys.(['E_ETS_' sreg]) = strys.(['E_' sreg]);
    ...
    for icoreg = 1:strpar.inbregions_p
        ...
        for icosec = 1:strpar.inbsectors_p
            ...
            strys.(['Q_A_F_' ssec '_' sreg]) = strys.(['Q_A_' ssec '_' sreg]) - strys.(['Q_A_I_' ssec '_' sreg]) - (strpar.(['sH_' sreg '_p']) * strys.Y+strys.(['I_PV_' sreg])) / strys.(['P_A_' ssec '_' sreg]) * (icosec == strpar.iSecHouse_p);
            if icosec == strpar.iSecEnergy_p && strpar.lCalibration_p == 1
                strys.(['Q_PV_' sreg]) = strpar.phiQPV0_p/(1-strpar.phiQPV0_p) * strys.(['Q_A_F_' ssec '_' sreg]);
                if strys.(['K_PV_' sreg])>0
                    strpar.phiPV_p = strys.(['Q_PV_' sreg])/strys.(['K_PV_' sreg]);
                end
            end
        end
