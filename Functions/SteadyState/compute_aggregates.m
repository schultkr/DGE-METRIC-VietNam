function [strys, strpar, strexo] = compute_aggregates(strys, strpar, strexo)
    % function [strys,strpar, strexo] = compute_aggregates(strys,strpar, strexo)
    ...
    for icoreg = 1:strpar.inbregions_p
        ...
        for icosec = 1:strpar.inbsectors_p
            ...
            for icosubsec = strpar.(['substart_' ssec '_p']):strpar.(['subend_' ssec '_p'])
                ssubsec = num2str(icosubsec);
                strys.(['I_' sreg]) = strys.(['I_' sreg]) + strys.(['I_' ssubsec '_' sreg])*strys.(['P_INV_' ssubsec '_' sreg])/strys.(['P_' sreg]);
            end
        end
